...
class F2SAConfig():
    ...
    def __init__(
        ...
        self.F2SA_init_lr_w = args.F2SA_init_lr_w if args.F2SA_init_lr_w is not None else args.init_lr
        self.F2SA_init_lr_u = args.F2SA_init_lr_u if args.F2SA_init_lr_u is not None else args.init_lr
        self.F2SA_init_lr_lambda = args.F2SA_init_lr_lambda if args.F2SA_init_lr_lambda is not None else args.init_lr
        ...
        self.freq_lambda = args.freq_lambda
        self.freq_w  = args.freq_w
        ...
        if args.F2SA_validation_sampler is not None:
            self.validation_sampler = args.F2SA_validation_sampler
        else:
            self.validation_sampler = args.sampler

class F2SA():
    def __init__(self, config: F2SAConfig):
    ...
    def run(
        ...
        record_df = df = pd.DataFrame(columns=['step', 'noisy_data_weight'])
        ...
        backbone_model = AutoModelForCausalLM.from_pretrained(config.model, torch_dtype=torch.bfloat16)
        ...
        model_w = AutoModelForCausalLM.from_pretrained(config.model, torch_dtype=torch.bfloat16)
        ...
        for i in range(num_outer_iter):
            ...
            for k in range(num_inner_iter):
                ...
                # We update w and u together
                lr = tau**(-i) * lr_scheduler(
                    init_lr=config.init_lr,
                    current_step=global_step // config.freq_lambda,
                    num_steps=num_outer_iter * num_inner_iter // config.freq_lambda,
                )
                lr_w = lr / config.init_lr * config.F2SA_init_lr_w
                update_optimizer_lr(optim_w, lr_w)
                update_optimizer_lr(optim_u, lr_w)
                optim_w.step()
                optim_u.step()
                ...
                stat_dict = {
                        'L1(w, lambda) + alpha L2(w,lambda)': loss_w_from_L1 + loss_w_from_L2,
                        'alpha L2(u,lambda)': loss_u_from_L2,
                ...
                # record the stats to the csv file
                noisy_indices = [v for k, v in partition_map.items() if 'noisy' in k]
                noisy_weight = sum(samp_prob_p.prob[i].item() for i in noisy_indices)

                new_row = pd.DataFrame({'step': [global_step], 'noisy_data_weight': [noisy_weight]})
                record_df = pd.concat([record_df, new_row], ignore_index=True)
                record_df.to_csv(config.csv_file, index=False)
                ...
                # Delay update of lambda
                if global_step % config.freq_lambda == 0:
                    lr = tau**(-i) * lr_scheduler(
                        init_lr=config.init_lr,
                        current_step=global_step // config.freq_lambda,
                        num_steps=num_outer_iter * num_inner_iter // config.freq_lambda,
                    )
                    lr_lambda = lr / config.init_lr * config.F2SA_init_lr_lambda
